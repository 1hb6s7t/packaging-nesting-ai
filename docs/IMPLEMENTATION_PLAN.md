...
- 正式镜像依赖审计留痕：`scripts/release_image_dependency_audit.py` 已输出 `policy_contract`，会校验 Dockerfile、镜像构建或显式复用、容器内依赖清单/复核审计生成、命令退出码、release-blocking 缺包装态、review-required 覆盖率和 dependency review 契约；`release_handoff_bundle.py` 已支持 `--release-image-dependency-audit`，可把 `release-image-dependency-audit.json` 和 release image 内生成的依赖清单/复核审计一起纳入 handoff，并会阻断正式镜像依赖契约失败；`go_live_readiness_audit.py` 会要求该总审计 artifact 通过。
