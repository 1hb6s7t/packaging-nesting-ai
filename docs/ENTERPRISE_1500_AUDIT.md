...
- Slow-gate continuation checks: `python scripts\enterprise_batch_slow_gates.py --output artifacts\enterprise-batch-slow-gates-full.json --batch-1500-count 1500 --batch-20000-count 20000 --real-sample-root "D:\大卖数智AI部\包装印刷\甘-包装样例" --hash-real-sample-files`; latest local full run passed with 21,500 generated files, 6 real sample cases, and 0 errors. Development tests use reduced counts through `tests/backend/test_enterprise_batch_slow_gates.py`.
