...
class ExternalCliSolverAdapter(SolverAdapter):
    ...
    def solve(self, job: NestingJob, config: SolverConfig) -> NestingSolution:
        ...
        result = run_json_cli(command, self.build_payload(job, config), timeout_sec=config.time_limit_sec)
        if result.status != "passed":
            return self.failed_solution(job, self.failure_reason(result), runtime_ms=result.duration_ms)
        try:
            payload = json.loads(result.stdout)
        except json.JSONDecodeError as exc:
            return self.failed_solution(job, f"{self.name.value} returned invalid JSON: {exc}", runtime_ms=result.duration_ms)
        try:
            return self.solution_from_payload(job, payload, runtime_ms=result.duration_ms)
        except (TypeError, ValueError, KeyError) as exc:
            return self.failed_solution(job, f"{self.name.value} certificate could not be parsed: {exc}", runtime_ms=result.duration_ms)
    ...
    def solution_from_payload(self, job: NestingJob, payload: dict[str, Any], *, runtime_ms: int) -> NestingSolution:
    ...
    def failed_solution(self, job: NestingJob, reason: str, *, runtime_ms: int = 0) -> NestingSolution:
        return NestingSolution(
